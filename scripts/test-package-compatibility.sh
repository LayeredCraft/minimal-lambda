...
build_consumer() {
  ...
  run_logged "build-$name-$target" dotnet build "$project" \
    --configuration Release \
    --framework "$target" \
    --no-restore \
    --nologo \
    ...
    "/p:CompilerGeneratedFilesOutputPath=$project_dir/obj/generated/$target"
  assert_warning_free "$LOGS/build-$name-$target.log"
  assert_generated_once "$project_dir" "$target"
}
...
for filename, (package_id, version, kind) in expected.items():
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if kind in {"core", "abstractions"}:
            ...
            assert_libs(names, package_id, ("net8.0", "net10.0"))
        ...
        if kind == "durable":
            ...
            if len(dependency_groups) != 2 or tfms != {"net8.0", "net10.0"}:
...
for filename in sys.argv[4:]:
    ...
    for framework in ("net8.0", "net10.0"):
        targets = [value for name, value in data["targets"].items() if name == framework]
        if not targets:
            raise SystemExit(f"{path}: missing assets target {framework}")
        if not any(expected <= set(target) for target in targets):
            raise SystemExit(f"{path}: wrong MinimalLambda versions for {framework}")
...
for target in net8.0 net10.0; do
  build_consumer typed "$TYPED" "$target"
  build_consumer task "$TASK" "$target"
done
...
restore_consumer invalid "$INVALID" "$CORE_VERSION"
rm -f "$LOGS/build-invalid.log"
set +e
dotnet build "$INVALID" \
  ...
  --nologo \
  --tl:off \
  "/flp:logfile=$LOGS/build-invalid.log;errorsonly;nosummary" \
  "/p:CorePackageVersion=$CORE_VERSION" \
  "/p:DurablePackageVersion=$DURABLE_VERSION" \
  >/dev/null 2>&1
invalid_status=$?
set -e
cat "$LOGS/build-invalid.log"
[ "$invalid_status" -ne 0 ] || fail "invalid-signature build unexpectedly succeeded"
invalid_count=$(grep -Eo 'error LH0007' "$LOGS/build-invalid.log" | wc -l | tr -d ' ')
[ "$invalid_count" = "1" ] || fail "expected exactly one LH0007, found $invalid_count"
invalid_ids=$(grep -Eo 'error [A-Z]+[0-9]+' "$LOGS/build-invalid.log" | awk '{print $2}' | sort -u | tr '\n' ' ' | sed 's/ $//')
[ "$invalid_ids" = "LH0007" ] || fail "invalid-signature build had unexpected diagnostics: ${invalid_ids:-none}"
grep -Fq 'found 0' "$LOGS/build-invalid.log" || fail "LH0007 did not report zero event parameters"
...
run_logged publish-aot dotnet publish "$AOT" \
  ...
  "/p:EmitCompilerGeneratedFiles=true" \
  "/p:CompilerGeneratedFilesOutputPath=$CONSUMERS/AotConsumer/obj/generated/net8.0"
assert_warning_free "$LOGS/publish-aot.log"
assert_generated_once "$CONSUMERS/AotConsumer" net8.0
